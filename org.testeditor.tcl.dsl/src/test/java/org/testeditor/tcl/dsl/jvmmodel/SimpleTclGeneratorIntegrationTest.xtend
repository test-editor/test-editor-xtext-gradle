...
class SimpleTclGeneratorIntegrationTest extends AbstractTclGeneratorIntegrationTest {
	...
	def void testStepWithQuestionMark() {
		...
		generatedCode.assertContains('''
			String IDvar2=nextSubId("ID"); reporter.enter(TestRunReporter.SemanticUnit.STEP, "Is <bar> visible ?", IDvar2, TestRunReporter.Status.STARTED, variables("<bar>", "Locator: label.greet in __synthetic0.aml:123", "@", "SimpleTest.tcl:7"));
	...
	def void interactionCallWithValueAndLocator() {
		...
		// then
		generatedCode.assertContains('''
			String IDvar2=nextSubId("ID"); reporter.enter(TestRunReporter.SemanticUnit.STEP, "Set value of <Input> to \"theValue\"", IDvar2, TestRunReporter.Status.STARTED, variables("<Input>", "Locator: text.input, Strategy: org.testeditor.dsl.common.testing.DummyLocatorStrategy.ID in __synthetic0.aml:127", "@", "SimpleTest.tcl:7"));
	...
	def void interactionCallWithValueAndLocatorInReverseOrder() {
		...
		// then
		generatedCode.assertContains('''
			String IDvar2=nextSubId("ID"); reporter.enter(TestRunReporter.SemanticUnit.STEP, "Set value \"theValue\" to <Input>", IDvar2, TestRunReporter.Status.STARTED, variables("<Input>", "Locator: text.input, Strategy: org.testeditor.dsl.common.testing.DummyLocatorStrategy.ID in __synthetic0.aml:127", "@", "SimpleTest.tcl:7"));
			dummyFixture.setValue("text.input", "theValue");
		'''.indent(3))
	}
